    ...
    return (
        <div className="group block flex-none w-40 sm:w-48 relative">
            <Card className="p-3 h-full flex flex-col justify-between !bg-white/5 hover:!bg-purple-500/10 transition-all duration-300">
                <div className="relative overflow-hidden rounded-lg mb-3 icon-glow-saffron bg-black/20 h-32 flex items-center justify-center">
...
const ProductDetailScreen: React.FC<ProductDetailScreenProps> = ({ products, addToCart }) => {
    const { t, wishlist, toggleWishlist, isPremiumActive, isAuthenticated, showAuth, currentUser, updateProfile } = useAppContext();
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            setTimeout(() => setIsCopied(false), 2000);
             ...
             prompt('Copy this link manually:', `${shareData.text}\n${shareData.url}`);
        }
    };
    ...
    const handlePremiumDownload = () => {
        ...
        if (!currentUser || !isPremiumActive) {
            setError('यह सुविधा केवल प्रीमियम सदस्यों के लिए है।');
            return;
        }
        ...
        const { limit } = getPremiumLimits();
        const currentDownloads = currentUser.downloadsConsumed || 0;

        if (currentDownloads >= limit) {
            setError(`आपने अपनी डाउनलोड सीमा (${limit} E-books) पूरी कर ली है। अधिक डाउनलोड करने के लिए कृपया अपना प्लान अपग्रेड करें।`);
            return;
        }
        ...
        // Update user profile with incremented download count
        updateProfile({ ...currentUser, downloadsConsumed: currentDownloads + 1 });
    };

    const { limit: downloadLimit, label: planLabel } = getPremiumLimits();
    const consumed = currentUser?.downloadsConsumed || 0;
    const remaining = Math.max(0, downloadLimit - consumed);
        ...
        <>
                ...
                >
                    ...
                    >
                        ...
                        {activeMedia === 'image2' && product.imageUrl2 && (
                            <img src={product.imageUrl2} alt={product.name} className="max-w-full max-h-full object-contain" />
                        )}
            ...
            <Card className="animate-fade-in mb-8">
                ...
                <div className="grid md:grid-cols-2 gap-8 items-start">
                    <div>
                        <div 
                            className="mb-4 cursor-pointer group relative rounded-lg overflow-hidden bg-black/20 h-[500px] flex items-center justify-center border border-white/10"
                        ...
                        >
                            ...
                            {activeMedia === 'image1' && <img src={product.imageUrl1} alt={product.name} className="w-full h-full object-contain" />}
                            {activeMedia === 'image2' && product.imageUrl2 && <img src={product.imageUrl2} alt={product.name} className="w-full h-full object-contain" />}
                            {activeMedia === 'video' && product.reviewVideoUrl && (
                                <video 
                        ...
                        <div className="flex gap-4 justify-center">
                            <button 
                                ...
                                className={`w-20 h-20 rounded-lg overflow-hidden border-2 bg-black/20 transition flex items-center justify-center ${activeMedia === 'image1' ? 'border-pink-400' : 'border-transparent'}`}
                            >
                                <img src={product.imageUrl1} alt="Product thumbnail 1" className="w-full h-full object-contain" />
                            </button>
                            {product.imageUrl2 && (
                                 <button 
                                    onClick={() => setActiveMedia('image2')}
                                    className={`w-20 h-20 rounded-lg overflow-hidden border-2 bg-black/20 transition flex items-center justify-center ${activeMedia === 'image2' ? 'border-pink-400' : 'border-transparent'}`}
                                >
                                    <img src={product.imageUrl2} alt="Product thumbnail 2" className="w-full h-full object-contain" />
                                </button>
                            )}
                    ...
                    <div>
                        ...
                        {isEbook && isPremiumActive && (
                            <div className="mb-6 p-4 bg-amber-900/40 rounded-xl border border-amber-500/50 flex flex-col gap-2">
                                ...
                                <div className="mt-2 bg-black/30 p-2 rounded-lg text-center border border-amber-500/30">
                                    <p className="text-xs text-amber-200 uppercase tracking-wide mb-1">{planLabel} सीमा</p>
                                    <div className="flex justify-center gap-6 font-mono text-sm text-white font-bold">
                                        <span>डाउनलोड हो गए: <span className="text-red-400">{consumed}</span></span>
                                        <span className="text-gray-500">|</span>
                                        <span>डाउनलोड बचे हैं: <span className="text-green-400">{downloadLimit > 9000 ? '∞ Unlimited' : remaining}</span></span>
                                    </div>
                                </div>
                            </div>
                        )}
