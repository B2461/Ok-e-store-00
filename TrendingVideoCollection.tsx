            ...
            <Card className="p-0 h-full overflow-hidden border-pink-500/30 !bg-gray-900 icon-glow-saffron active:!bg-amber-500/20 active:!border-amber-400 active:shadow-[0_0_15px_rgba(251,191,36,0.3)] transition-all duration-300">
                ...
                >
                    ...
                    <button
                        ...
                        className="absolute top-2 right-2 z-10 p-1.5 rounded-full bg-black/40 backdrop-blur-md hover:bg-pink-600/80 transition-all"
                    >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 pointer-events-none bg-gradient-to-t from-black via-black/80 to-transparent pt-6">
                    <h3 className="text-white font-hindi font-bold text-xs truncate shadow-black drop-shadow-md">{product.name}</h3>
                    <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-pink-400 font-bold text-sm drop-shadow-md">₹{(product.mrp - (product.mrp * product.discountPercentage / 100)).toFixed(0)}</span>
                        <span className="text-gray-400 text-[10px] line-through drop-shadow-md">₹{product.mrp}</span>
                    </div>
                </div>
            </Card>
        </div>
...
const TrendingVideoCollection: React.FC<TrendingVideoCollectionProps> = ({ products }) => {
    ...
    return (
        <div className="w-full mb-12 animate-fade-in mt-4">
            ...
            {/* Changed from horizontal scroll to Grid (2 columns on mobile) */}
