...
const SocialMediaVisibilityManager: React.FC<{
    ...
    const socialPlatforms = [
        ...
        { id: 'atoplay', label: 'Atoplay' },
        { id: 'arratai', label: 'Arratai' },
    ...
    const handleToggle = (platformId: string) => {
        onUpdateVisibility({
            ...visibility,
            [platformId]: !(visibility[platformId] ?? true) // Default to true if not set
        });
    };

    return (
        <Card>
            <p className="text-center text-purple-300 mb-6">चुनें कि कौन से सोशल मीडिया आइकन शीर्ष बार पर दिखाई देंगे।</p>
            <div className="max-h-[60vh] overflow-y-auto space-y-4 pr-2">
                {socialPlatforms.map(platform => (
                    <div key={platform.id} className="flex justify-between items-center p-4 bg-white/5 rounded-lg border border-white/20">
                        <span className="font-semibold text-white">{platform.label}</span>
                        <div
                            onClick={() => handleToggle(platform.id)}
                            className={`auto-pay-toggle ${(visibility[platform.id] ?? true) ? 'active' : ''}`}
                            role="switch"
                            aria-checked={visibility[platform.id] ?? true}
...
const AdminPanel: React.FC<AdminPanelProps> = (props) => {
    const { products, onUpdateProducts, orders, onUpdateOrders, pendingVerifications, onApproveVerification, supportTickets, onUpdateTicket, socialMediaPosts, onCreatePost, onUpdatePost, onDeletePost, categoryVisibility, onUpdateCategoryVisibility, socialVisibility, onUpdateSocialVisibility } = props;
    ...
    const SectionHeader = ({ id, label, count }: { id: string, label: string, count?: number }) => (
        <button 
            ...
            className={`w-full flex items-center justify-between p-4 rounded-lg transition-all duration-300 ${activeTab === id ? 'bg-purple-600 text-white shadow-lg' : 'bg-white/10 text-purple-200 hover:bg-white/20'}`}
        >
            <span className="text-xl font-hindi font-bold">{label}</span>
            <div className="flex items-center gap-3">
                ...
                <span className={`transform transition-transform duration-300 ${activeTab === id ? 'rotate-180' : ''}`}>
                    ▼
                </span>
            </div>
        </button>
    ...
    return (
        <div className="animate-fade-in w-full max-w-6xl mx-auto space-y-4">
            
            {/* Payment Verifications */}
            <SectionHeader id="verifications" label="भुगतान सत्यापन" count={pendingVerifications.length} />
            ...
            {/* Order Management */}
            <SectionHeader id="orders" label="ऑर्डर प्रबंधन" count={orders.filter(o => o.status === 'Processing' || o.status === 'Verification Pending').length} />
            {activeTab === 'orders' && (
                <div className="animate-fade-in">
            ...
            {/* Support Tickets */}
            <SectionHeader id="tickets" label="सहायता टिकट" count={supportTickets.filter(t => t.status === 'Open').length} />
            {activeTab === 'tickets' && (
                <div className="animate-fade-in">
            ...
            <SectionHeader id="social_media" label="सोशल मीडिया पोस्ट" />
            ...
            {/* Social Media Visibility Control - NEW */}
            <SectionHeader id="social_control" label="सोशल मीडिया कंट्रोल" />
            {activeTab === 'social_control' && (
                <div className="animate-fade-in">
                    <SocialMediaVisibilityManager
                        visibility={socialVisibility}
                        onUpdateVisibility={onUpdateSocialVisibility}
                    />
                </div>
            )}

            {/* Local Marketing */}
            <SectionHeader id="local_marketing" label="स्थानीय विपणन" />
            ...
            {/* All India Marketing */}
            <SectionHeader id="all_india_marketing" label="अखिल भारतीय विपणन" />
            ...
            {/* Category Control */}
            <SectionHeader id="category_control" label="कैटेगरी कंट्रोल" />
            ...
            {/* Product Management */}
            <SectionHeader id="products" label="उत्पाद प्रबंधन" />
            {activeTab === 'products' && (
