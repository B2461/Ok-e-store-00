...
const CheckoutScreen: React.FC<CheckoutScreenProps> = ({ cartItems, onPlaceOrder, onVerificationRequest }) => {
    ...
    const handleSelectPaymentMethod = (method: 'PREPAID' | 'COD') => {
        ...
        if (method === 'COD') {
            ...
            // Generate detailed items string with Color and Size
            const itemsSummaryWa = cartItems.map(item => 
                `- ${item.name} (Qty: ${item.quantity}${item.selectedColor ? `, Color: ${item.selectedColor}` : ''}${item.selectedSize ? `, Size: ${item.selectedSize}` : ''})`
            ).join('\n');

            const addressDetails = hasPhysicalItems 
...
${itemsSummaryWa}`;
    ...
    const handleSubmitForVerification = async (e: FormEvent) => {
        ...
        try {
            ...
            // Generate detailed items string with Color and Size for WhatsApp
            const itemsSummaryWa = cartItems.map(item => 
                `- ${item.name} (Qty: ${item.quantity}${item.selectedColor ? `, Color: ${item.selectedColor}` : ''}${item.selectedSize ? `, Size: ${item.selectedSize}` : ''})`
            ).join('\n');
